document.addEventListener('DOMContentLoaded', () => {
    ...
    const getFilteredProducts = (category, searchTerm = '') => {
    ...
    const populateProductGrid = (category, searchTerm = '', page = 1) => {
    ...
    const updatePagination = (currentPage, totalPages, totalProducts) => {
        ...
        if (totalPages > 1) {
            ...
            prevBtn.onclick = () => {
                if (currentPage > 1) {
                    currentPage--;
                    populateProductGrid(currentCategory, searchInput.value, currentPage);
                }
            };
    ...
    const createCategoryButtons = () => {
        ...
        // Filter out 'all' and special categories, keep only main categories
        const mainCategories = productCategories.filter(cat => 
            !['all', 'new-arrivals', 'trending', 'highly-rated'].includes(cat.id)
        );

        mainCategories.forEach(category => {
            const button = document.createElement('button');
            button.className = `btn category-btn ${category.id === currentCategory ? 'active' : ''}`;
    ...
    const updateCategoryDisplay = (category) => {
        ...
        categoryDescription.textContent = `Browse our ${category.name.toLowerCase()} collection`;
        productsTitle.textContent = `${category.name} Products`;
    };
    ...
    const performSearch = () => {
        ...
        currentPage = 1;
        populateProductGrid(currentCategory, searchTerm, currentPage);
    };
    ...
    const showRelatedProducts = (currentProduct) => {
        ...
        const relatedProducts = productsData
            .filter(p => p.id !== currentProduct.id && p.category.includes(currentCategory))
            .slice(0, 3);
